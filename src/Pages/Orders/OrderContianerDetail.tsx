...
export default function OrderContainerDetail({ order, minimize }: { order: any, minimize: () => void }) {
    ...
    return (
        <div >
        <div className="p-4 m-4 border border-gray-300 dark:border-gray-500 rounded" id="print"  ref={componentRef}>
            ...
            <div className={`flex w-full justify-between mt-2 pb-2 `}>
                    ...
                    <div className="flex gap-2">
                        <div className="flex flex-col">
                            <div className="text-gray-400 dark:text-gray-300">#{order?.id?.split('-')[0]} • {order.createdAt ? format(new Date(order.createdAt), 'do MMM yyyy') : ''}</div>
                            <div className="text-gray-600 dark:text-gray-200 my-1">{order.supplier?.name} {'->'} {order?.buyer?.name}</div>
                            <div className="flex w-max bg-green-200 font-bold text-sm text-green-800 px-2 rounded items-center">{order?.orderStatus?.[0]?.note}</div>
                        </div>
                    </div>
                    <div className="flex">
                        <div className="flex flex-col">
                            <div className="text-gray-600 text-lg font-extrabold dark:text-gray-200">₹ {(parseFloat(order?.totalAmount) - parseFloat(order?.flatDiscount)).toFixed(2)}</div>
                            <div className="text-gray-400 text-xs font-extrabold mx-auto dark:text-gray-300">({order?.numberOfItems} items)</div>
                        </div>
                    </div>
                </div>
